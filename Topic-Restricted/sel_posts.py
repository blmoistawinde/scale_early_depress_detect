...
for id0, posts, label in tqdm(zip(user_ids_train, user_posts_train, user_labels_train), total=len(user_ids_train)):
    ...
    with open(f"{in_dir}/last_sim{topK}/train/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in posts[-topK:]))
    ...
    with open(f"{in_dir}/depress_sim{topK}/train/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
    # questionaire only
    sim_scores = pair_sim[:, 3:].max(1)
    top_ids = sim_scores.argsort()[-topK:]
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/questionaire_sim{topK}/train/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
    # combined
    ...
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/combined_sim{topK}/train/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
...
for id0, posts, label in tqdm(zip(user_ids_val, user_posts_val, user_labels_val), total=len(user_ids_val)):
    posts = [x.replace("<NEGATE_FLAG>", "").replace("[ removed ]", "").strip() for x in posts]
    posts = [x for x in posts if len(x) > 0]
    with open(f"{in_dir}/last_sim{topK}/val/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in posts[-topK:]))
    embs = sbert.encode(posts)
    pair_sim = cosine_similarity(embs, combined_embs)
    # depression only
    ...
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/depress_sim{topK}/val/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
    # questionaire only
    sim_scores = pair_sim[:, 3:].max(1)
    top_ids = sim_scores.argsort()[-topK:]
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/questionaire_sim{topK}/val/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
    # combined
    sim_scores = pair_sim.max(1)
    top_ids = sim_scores.argsort()[-topK:]
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/combined_sim{topK}/val/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
...
for id0, posts, label in tqdm(zip(user_ids_test, user_posts_test, user_labels_test), total=len(user_ids_test)):
    posts = [x.replace("<NEGATE_FLAG>", "").replace("[ removed ]", "").strip() for x in posts]
    posts = [x for x in posts if len(x) > 0]
    with open(f"{in_dir}/last_sim{topK}/test/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in posts[-topK:]))
    embs = sbert.encode(posts)
    pair_sim = cosine_similarity(embs, combined_embs)
    # depression only
    ...
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/depress_sim{topK}/test/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
    # questionaire only
    sim_scores = pair_sim[:, 3:].max(1)
    top_ids = sim_scores.argsort()[-topK:]
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/questionaire_sim{topK}/test/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
    # combined
    sim_scores = pair_sim.max(1)
    top_ids = sim_scores.argsort()[-topK:]
    top_ids = np.sort(top_ids)  # sort in time order
    sel_posts = [posts[ii] for ii in top_ids]
    with open(f"{in_dir}/combined_sim{topK}/test/{id0}_{label}.txt", "w") as f:
        f.write("\n".join(x.replace("\n", " ") for x in sel_posts))
